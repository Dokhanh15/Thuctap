...
const Register = () => {
  const {
    ...
    watch,
  } = useForm<Users & { confirmPassword: string }>({ mode: 'onChange' });
  ...
  const onSubmit: SubmitHandler<Users & { confirmPassword: string }> = async (data) => {
    try {
      ...
      toast.error((error as AxiosError)?.message || "Đã xảy ra lỗi. Vui lòng thử lại.");
      ...
      <div className="bg-white p-8 rounded-lg border border-gray-300 shadow-md w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            type="submit"
            disabled={!isValid}
            className={`w-full py-2 px-4 ${isValid ? "bg-indigo-800" : "bg-gray-400"} text-white font-semibold rounded-md shadow-lg hover:bg-indigo-700 transition-all duration-300`}
          >
            Đăng ký
          </button>

          <div className="flex justify-end mt-4">
            <a href="/login" className="text-sm text-indigo-800 hover:underline">Đã có tài khoản? Đăng nhập</a>
          </div>
        </form>
      </div>
    </div>
